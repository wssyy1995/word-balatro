  ...
  draw(ctx, game, W, H, s) {
    ...
    for (let i = 0; i < actualWitchSlots; i++) {
      ...
      if (isSelling && joker) {
        ...
        if (game._jokerSortState && game._jokerSortState.fromIndex === i) {
          ...
          const jGrad = ctx.createRadialGradient(jCx, jCy, Math.min(drawW, drawH) * 0.32, jCx, jCy, jRadius);
          jGrad.addColorStop(0, 'rgba(162, 89, 255, 0)');
          jGrad.addColorStop(0.55, `rgba(162, 89, 255, ${0.08 * jBreath})`);
          jGrad.addColorStop(1, `rgba(162, 89, 255, ${0.22 * jBreath})`);
          ctx.fillStyle = jGrad;
          ctx.beginPath();
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (isSelling && potion) {
        ...
        const pGrad = ctx.createRadialGradient(pCx, pCy, Math.min(slotW, oSlotH) * 0.32, pCx, pCy, pRadius);
        pGrad.addColorStop(0, 'rgba(80, 220, 120, 0)');
        pGrad.addColorStop(0.55, `rgba(80, 220, 120, ${0.08 * pBreath})`);
        pGrad.addColorStop(1, `rgba(80, 220, 120, ${0.22 * pBreath})`);
        ctx.fillStyle = pGrad;
        ctx.beginPath();
