...
const { DICTIONARY, SHOP_POOL, onlineWordCache } = require('./data');
...
class Renderer {
  ...
  drawPlaying(game) {
    ...
    if (selected.length >= 3) {
      ...
      const inLocal = DICTIONARY.has(word);
  ...
  drawPotion(game) {
    ...
    // 标题根据药水类型显示不同提示
    const effect = game.potionMode ? game.potionMode.effect : '';
    if (effect === 'replicate_letter') {
      if (game.potionMode.firstCardId) {
        this.text('选择第二张牌进行复刻', W / 2, 70 * s, 18, '#fff');
      } else {
    ...
    game.hand.forEach((card, i) => {
      ...
      // 判断是否是已选中的第一张牌
      const isFirstSelected = game.potionMode && game.potionMode.firstCardId === card.id;
      ...
      this.roundRect(x, y, cardW, cardH, 6 * s, bg, stroke);
      this.text(card.letter, x + cardW / 2, y + cardH * 0.35, 22, '#fff');
      this.text(`${card.score}分`, x + cardW / 2, y + cardH * 0.7, 10, '#bdc3c7');
      ...
      if (isFirstSelected) {
        ...
        ctx.fillStyle = '#f1c40f';
        ctx.textAlign = 'center';
        ctx.fillText('已选', x + cardW / 2, y + cardH * 0.55);
      }
