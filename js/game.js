...
function findAllValidWordsInHand(hand) {
  ...
  for (let len = 3; len <= Math.min(9, letters.length); len++) {
    ...
    for (const p of gens) {
      ...
      seen.add(word);
      if (DICTIONARY.has(word) || onlineWordCache.has(word)) {
        const indices = [];
        const used = new Set();
        for (const ch of p) {
...
function isValidWord(word) {
  ...
  return DICTIONARY.has(word) || onlineWordCache.has(word);
}
...
class Game {
  ...
  upgradeCard(cardId) {
    ...
    // 字母复刻：需要选择两张牌
    if (effect === 'replicate_letter') {
      if (!this.potionMode.firstCardId) {
        // 选择第一张牌
        const card = this.hand.find(c => c.id === cardId);
        if (!card) return false;
        this.potionMode.firstCardId = cardId;
        if (this.audioManager) this.audioManager.play('select');
        return true;
      }

      // 选择第二张牌
      if (this.potionMode.firstCardId === cardId) return false; // 不能选同一张
      const firstCard = this.hand.find(c => c.id === this.potionMode.firstCardId);
      const secondCard = this.hand.find(c => c.id === cardId);
      if (!firstCard || !secondCard) return false;

      // 比较分数
      const highCard = firstCard.score >= secondCard.score ? firstCard : secondCard;
      const lowCard = firstCard.score >= secondCard.score ? secondCard : firstCard;

      // 80%概率低分变高分，20%高分变低分
      const success = Math.random() < 0.8;
      if (success) {
        // 低分牌复制高分牌的分数
        lowCard.score = highCard.score;
        lowCard.upgradeMult = highCard.score / lowCard.baseScore;
        lowCard.upgraded = true;
      } else {
        // 高分牌被拉低到低分牌的分数
        highCard.score = lowCard.score;
        highCard.upgradeMult = lowCard.score / highCard.baseScore;
