...
弃牌或出牌后，旧牌回到牌堆底部并重新洗牌，然后通过 `drawWithVowelRules` 补牌：

- 从牌堆顶部抽取等量新牌，检查**保留手牌 + 新抽牌**的元音分布
- 必须满足：**至少 2 种不同元音**，且**每种元音不超过 2 张**
...
**其他约束**
- 回合初始发牌始终执行双种子词逻辑
- 弃牌/出牌后补牌执行 `drawWithVowelRules` 元音规则，不额外调用保底检查
...
| v1.9.7 | 2026-06-11 | 目标分数系数全面下调：2~5关20、6~10关25、11~20关30、21~30关35、31~40关40、41~50关45、51+关50；同步更新 README 目标分数表 |
