...
- **危险复制**：选择两个字母，60% 概率低分变高分，40% 概率相反；选中两个字母后播放约 1 秒旋转动画并揭晓结果，成功后目标字母分数永久替换为源字母分数
...
3. **匹配成功**：随机生成对手（昵称与头像索引绑定），进入 matched 阶段展示对手信息；1.5 秒后进入 countdown 3 秒倒计时；最后 disappearing 淡出并正式进入对战。
   - **对手头像来源**：取自 `rank_avatar` 云图集，共 4 张 5×5 精灵图（每张 200×200，单头像 40×40），按索引 `idx` 计算所在图集、行、列，裁剪出 40×40 区域后圆形 clip 绘制。绘制逻辑与全国榜默认头像保持一致，不对单头像再做额外边距裁剪。
4. **对战回合**：
