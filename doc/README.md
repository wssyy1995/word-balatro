...
| 5（退场后） | 「获得女巫牌」弹窗 | 蒙层保留，屏幕中央弹出：标题「获得女巫牌」（金色装饰线）+ `has_vowel` 卡牌（等比缩放 + 圆角裁剪 + 金色光晕）+ 卡牌名称/介绍（金色半透明蒙层）+「领取」按钮（水波纹）；样式与女巫奖励弹窗一致，弹出时播放 `magic_twinkle` 音效。点击「领取」后引导全部完成（`guidePhase = 6`） |
