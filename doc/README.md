...
| 左右开弓 | `left_right_open` | per_card | 单词首尾两张字母牌 | 首尾字母各 score +40 |
...
| 类型 | 数量 | 价格 | 上限 | 标识色 |
|------|------|------|------|--------|
| **女巫牌**（witch） | 17 种 | 6-14 金币 | 装备栏默认 4 格（可扩展） | 紫色 |
...
> 部分女巫牌设有 `min_level` 解锁门槛，例如双子合影（Lv.20）、字母之神（Lv.10）等；低回合商店不会刷出高等级牌。`length_4/5/6`（四/五/六字母连击）当前在 `SHOP_POOL` 中已注释掉，商店暂不投放。
