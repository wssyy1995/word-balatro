...
首次进入游戏的玩家会在第 1 回合触发新手引导，共 **2 个内容 Phase + 退场**：
...
1600ms      → 女巫从左侧缓慢飞入（1200ms easeOutCubic，全程上下漂浮 ±6*s）
2800ms      → 女巫到位，聚光灯高亮字母卡牌区域
2900ms      → 对话框从右侧飞入（500ms easeOutCubic）
3900ms      → 文字开始逐字显示（每 65ms 一个字）
```
