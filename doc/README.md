...
```
target = 250 + Σ(第 r 关系数 × (r - 1))  (r 从 2 到当前回合)
```
...
每回合初始发牌时，固定生成两个种子词，确保玩家手牌中始终有合法单词可出：

- **双种子词固定长度**：一个长度 **3** 的单词 + 一个长度 **4** 的单词
- **种子词元音限制**：
  1. 两个种子词所有字母加起来，**不同元音种类不超过 2 个**（A/E/I/O/U）
  2. 同一个元音字母在两个种子词中**累计出现次数不超过 2 次**
- **种子词不从牌堆消耗**：种子词卡牌直接凭空创建，不消耗 `deck` 中的牌
- **种子词在手牌中打散**：7 张种子牌先内部打乱顺序，再逐个随机穿插到手牌各位置，不按单词字母顺序连续出现

**随机补牌规则**

在种子词之外，手牌剩余空位由随机牌补足（默认 9 张手牌补 2 张）：
...
2. 在词库中查找包含这 2 个字母的保底种子词（出牌默认长度 4，弃牌默认长度 3；`force_contain_X` 约束下会优先生成含指定字母的种子词）
...
> ⚠️ 当前临时调整：女巫约束仍按上表在各关卡生效，但**女巫奖励阶段（`witch_reward`）仅在第 3 关触发**，第 5 关及以后通关后不再进入奖励阶段。
>
...
- **复刻水**：选择两个字母，60% 概率低分变高分，40% 概率相反；选中两个字母后播放约 1 秒旋转动画并揭晓结果，成功后目标字母分数永久替换为源字母分数
- **平分秋色**：选择两个字母，将当前分数相加后平分，永久修改两字母基础分
- **星辉洗涤**：选择一个字母，重置所有强化恢复基础分，并获得当前分数 1/10 的金币

> **吸星大法** 不进入药水升级页，购买"立即使用"或从道具栏点击后，在主玩法中选择一张手牌，将其他手牌分数临时加给它；被弃掉的牌 `absorbBonus` 立即清零。
...
> - **立即使用且不占药水槽**（药水槽满时仍可购买，但"暂存"按钮置灰）：字母升级、随机强化、复刻水、平分秋色、星辉洗涤。
> - **复刻水 / 平分秋色 / 星辉洗涤**：进入全屏选择页，选中字母后点击"开始"，播放约 1 秒旋转动画并揭晓结果；复刻水成功后低分字母分数永久替换为高分字母分数，失败则相反；平分秋色永久生效；星辉洗涤重置强化并立即发放金币。
...
点击价格按钮
  ...
  → 显示购买成功弹窗
     ...
     └── 药水牌 → 点击"暂存" → 加入 potions[]（字母置换、吸星大法不可暂存，只能在游戏中直接使用）
                → 点击"立即使用" → 进入 potion 状态（或立即生效）
                    ├── 复刻水 → 全屏选择两个字母后开奖
                    ├── 平分秋色 → 全屏选择两个字母后平分
                    ├── 星辉洗涤 → 全屏选择一个字母后重置并领取金币
                    └── 吸星大法 → 返回主玩法，选择一张手牌吸收其他手牌分数
...
- 首次装备第 2 张女巫牌时，已装备的两张牌会一起轻微抖动 3 秒，提示玩家可以排序
