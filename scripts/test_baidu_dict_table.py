    ...
    for word in TEST_WORDS:
        try:
            ...
            results.append({
                ...
                'dst': '', 'phEn': '', 'phAm': '', 'wordMeans': [], 'parts': [], 'reason': str(e)
            })

    # 输出 Markdown 表格
    print("| 单词 | 有效? | from | 翻译 | 音标(英) | 音标(美) | word_means | 词性+释义 |")
    print("|------|-------|------|------|----------|----------|------------|-----------|")
    for r in results:
        ...
        print(f"| {escape_md(word)} | {valid} | {escape_md(from_val)} | {escape_md(dst)} | {escape_md(phEn)} | {escape_md(phAm)} | {escape_md(wordMeans)} | {escape_md(parts_str)} |")
